...
document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
        ...
        if (initialDataElement) {
            const jsonData = initialDataElement.textContent.trim();
            console.log('Raw JSON data:', jsonData); // 디버깅을 위해 raw JSON 데이터 출력
            existingImageUrls = JSON.parse(jsonData);
            console.log('Parsed JSON data:', existingImageUrls); // 디버깅을 위해 파싱된 JSON 데이터 출력
        }
    } catch (e) {
    ...
    selectedFiles = existingImageUrls.map(url => ({ name: url.split('/').pop(), dataURL: url }));
    localStorage.setItem('selectedFiles', JSON.stringify(selectedFiles));
