...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadMorePosts() {
        ...
        const goodRestaurant = goodRestaurantCheckbox.checked; // 체크박스 상태 가져오기
        const sortOption = sortSelect.value || "date"; // 정렬 옵션 가져오기 (기본값: 최신순)
        fetch(`/post?page=${currentPage}&size=${postsPerPage}&goodRestaurant=${goodRestaurant}&latitude=${latitude != null ? latitude : ''}&longitude=${longitude != null ? longitude : ''}&sortOption=${sortOption}`)
            .then(response => response.text())
            .then(html => {
                const postList = document.querySelector("#postList ul");
                if (postList) {
    ...
    goodRestaurantCheckbox.addEventListener('change', function () {
        ...
        const goodRestaurant = goodRestaurantCheckbox.checked; // 체크박스 상태 가져오기
        const sortOption = sortSelect.value || "date"; // 정렬 옵션 가져오기 (기본값: 최신순)
        fetch(`/post?page=0&size=${postsPerPage}&goodRestaurant=${goodRestaurant}&latitude=${latitude != null ? latitude : ''}&longitude=${longitude != null ? longitude : ''}&sortOption=${sortOption}`)
            .then(response => response.text())
            .then(html => {
                const postList = document.querySelector("#postList ul");
                if (postList) {
